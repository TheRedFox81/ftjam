...
macro_headers( TARGET *t )
{
    ...
    while( fgets( buf, sizeof( buf ), f ) )
    {
        ...
        if ( regexec( re, buf ) && re->startp[1] )
        {
            ...
            v->filename = 0;
            v = hashput( header_macros_hash, (const char*)buf1, &create );
            if (create) {
                v->symbol   = newstr( buf1 );
                v->filename = newstr( buf2 );
            }
...
macro_header_get( const char*  macro_name )
{
  ...
  if( header_macros_hash && (v = hashget( header_macros_hash, macro_name )) != NULL )
  {
    if ( DEBUG_HEADER )
